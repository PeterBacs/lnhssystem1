...
class ResultRepository
{
    public function result($exam_type_id)
    {
        ...
        $subject_ids            = MarksRegister::query()
                                ...
                                ->where('classes_id', $sessionClassStudent->classes_id)
                                ->where('section_id', $sessionClassStudent->section_id)
                                ->whereHas('marksRegisterChilds', function ($query) {
        ...
        $percentage             = (int) number_format(($totalMark / $allSubjectTotalMark) * 100, 2);
        ...
        $data['gpa']            = $gpa;
        $data['marksheet_pdf']  = route('student.marksheet-pdf', ['exam_type_id' => $exam_type_id]) . '?student_id=' . @auth()->user()->student->id;
